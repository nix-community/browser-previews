{ newScope, config, stdenv, fetchurl, makeWrapper, buildPackages
...
, channel ? "stable", upstream-info ? (import ./upstream-info.nix).${channel}
, proprietaryCodecs ? true, enableWideVine ? false
, ungoogled ? false # Whether to build chromium or ungoogled-chromium
, cupsSupport ? true, pulseSupport ? config.pulseaudio or stdenv.isLinux
, commandLineArgs ? "", pkgsBuildTarget, pkgsBuildBuild, pkgs }:
...
let
  ...
  warnObsoleteVersionConditional = min-version: result:
    let
      min-supported-version = (lib.head
        (lib.attrValues electron-source)).unwrapped.info.chromium.version;
    in lib.warnIf (lib.versionAtLeast min-supported-version min-version)
  ...
  chromeSrc = let
    ...
    version = if chromium.upstream-info.hash_deb_amd64 != null then
      chromium.upstream-info.version
    else
      (import ./upstream-info.nix).stable.version;
    hash = if chromium.upstream-info.hash_deb_amd64 != null then
  ...
  suffix =
    lib.optionalString (channel != "stable" && channel != "ungoogled-chromium")
    ("-" + channel);
